...
class Transformers:
    ...
    def match(self, text_1: str, text_2: str) -> float:
        embeddings = self.model.encode([self.lema.preprocess_text(text_1), self.lema.preprocess_text(text_2)])
        return sentence_transformers.util.cos_sim(a=embeddings[0], b=embeddings[1]).item()*100
