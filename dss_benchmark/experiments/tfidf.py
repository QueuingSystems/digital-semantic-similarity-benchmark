...
def tfidf_match(
    matcher: TfIdf, cutoff: int, dataset: List[Datum], verbose=False
):
...
def tfidf_experiment(
    ...
    result2 = _research_bin_and_sublinear_tf(dataset, params1, key, dataset_name, results_folder, verbose)
    params2 = params1
...
def _research_ngram(
    dataset: List[Datum], dataset_name: str, key: str, results_folder: str, verbose=False
):
    data = []
...
def _research_bin_and_sublinear_tf(
     dataset: List[Datum], params: TfIdfParams, key: str, dataset_name: str,  results_folder: str, verbose=False   
):
    data = []
    list_bin= [False, True]
    ...
    df = pd.DataFrame(data)
    df.to_csv('{}/tfidf_{}_ngram.csv'.format(
        results_folder,
        dataset_name)) 
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    groups = df[[key, "cutoff", "binary", "sublinear_tf"]].groupby(["binary", "sublinear_tf"])
    for _, group in groups:
        group = group.reset_index()
        group[[key, "cutoff", "binary", "sublinear_tf"]].plot(
            ...
            kind="line",
            ax=ax,
            label='binary={} sublinear_tf={}'.format(group['binary'][0], group['sublinear_tf'][0]),
        )
    ax.set_xlabel("tf-idf cutoff")
    ax.set_ylabel(key)
...
def _tfidf_roc_curve(
    ...
    options = [
        (param, 'tf-idf ngram=({},{}) binary={} sublinear-tf={}'.format(
             param.min_ngram, param.max_ngram, 
            param.binary, param.sublinear_tf))
        for param in params
    ]
