...
@benchmarks.command(
    help='Посмотреть параметры бенчмарка'
)
@click.option(
...
def params(dataset_name):
    ...
    iqr_text_length_1 = np.percentile([len(x.text_1) for x in dataset], 75) - np.percentile([len(x.text_1) for x in dataset], 25)
    iqr_text_length_2 = np.percentile([len(x.text_2) for x in dataset], 75) - np.percentile([len(x.text_2) for x in dataset], 25)
