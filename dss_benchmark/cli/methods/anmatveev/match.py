...
@click.option("-imp", "--image_path", required=True, type=str, help="Путь к графику", prompt=True)
@click.argument(
    "args", nargs=-1, type=click.UNPROCESSED
)
def max_f1(model_path, texts, text1, text2, image_path, args=None):
    kwargs = parse_arbitrary_arguments(args)
    params = TrainModelParams(**kwargs)
    manager = MatchManager(model_path, args)
    ...
    plotManager.save(image_path=image_path)
    return res
